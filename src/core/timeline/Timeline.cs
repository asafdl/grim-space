...
public sealed class Timeline
{
	...
	public Timeline() => Clock = new TickClock(_sync);
	...
	{
		lock (_sync)
			return HistoryCore(tick);
	}

	public IReadOnlyList<TimelineBatch> DrainUntil(int tick)
	{
		lock (_sync)
			return DrainUntilCore(tick);
	}

	internal void TrimHistory(int retainTicks)
	{
		ArgumentOutOfRangeException.ThrowIfNegative(retainTicks);
		if (retainTicks == 0)
			return;

		lock (_sync)
		{
			var cutoff = Clock.Current - retainTicks;
			if (cutoff <= 0)
				return;

			DrainUntilCore(cutoff);
		}
	}
	...
	public IReadOnlyDictionary<string, IReadOnlyList<IAction>> HistoryByActor(int? tick = null)
	{
		lock (_sync)
		{
			...
			foreach (var entry in HistoryCore(tick))
			{
	...
	private IReadOnlyList<ITimelineEntry> HistoryCore(int? tick) =>
		_history.TryGetValue(tick ?? Clock.Current, out var entries) ? entries : [];

	private List<TimelineBatch> DrainUntilCore(int tick)
	{
