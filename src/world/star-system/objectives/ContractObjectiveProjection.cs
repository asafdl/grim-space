...
public static class ContractObjectiveProjection
{
	...
	private static ObjectiveSummaryContent BuildSummary(StarMap map, Contract contract)
	{
		if (contract.Objective is not HuntObjective hunt
			...
			return PlainOrPreview(contract);
	...
	private static ObjectiveSummaryContent PlainOrPreview(Contract contract) =>
		new ObjectiveSummaryContent.Plain(ContractDisplay.ObjectivePreview(contract));
}
